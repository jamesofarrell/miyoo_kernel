...
static int h_front_porch = 8;
static int h_back_porch = 8;
static int h_sync_len = 1;
static int v_front_porch = 8;
static int v_back_porch = 8;
static int v_sync_len = 1;
static int h_offset = 0;
static int v_offset = 0;
static int vram_size_mult = 1;
static int lram_size_mult = 1;
static int total_mult = 2;
static int total_sum = 0;
module_param(h_front_porch, int, 0660);
module_param(h_back_porch, int, 0660);
module_param(h_sync_len, int, 0660);
module_param(v_front_porch, int, 0660);
module_param(v_back_porch, int, 0660);
module_param(v_sync_len, int, 0660);
module_param(h_offset, int, 0660);
module_param(v_offset, int, 0660);
module_param(vram_size_mult, int, 0660);
module_param(lram_size_mult, int, 0660);
module_param(total_mult, int, 0660);
module_param(total_sum, int, 0660);
...
static int def_bp=8;
static int def_fp=8;
module_param(new_bp, int, 0660);
module_param(new_fp, int, 0660);
module_param(def_bp, int, 0660);
module_param(def_fp, int, 0660);
...
static void suniv_lcdc_init(struct myfb_par *par)
{
  ...
  total = par->mode.xres * 1 + h_front_porch + bp + h_offset;
  writel(((total - 1) << 16) | ((bp - 1) << 0), iomm.lcdc + TCON0_BASIC_TIMING_REG1);
  bp = v_sync_len + v_back_porch;
  total = par->mode.yres + v_front_porch + bp + v_offset;
  writel((((total * total_mult) + total_sum) << 16) | ((bp - 1) << 0), iomm.lcdc + TCON0_BASIC_TIMING_REG2);
...
static int myfb_probe(struct platform_device *device)
{
  ...
  par->vram_size = 320*240*2*2 * vram_size_mult;
  ...
  par->lram_size = 320*240*18 * lram_size_mult;
  for(i=0; i<LRAM_NUM; i++){
